App.namespace 'App.views.widgets.tilegrid', require: [

  'views/widgets/tilegrid'
  ...
  ###
    ...
    - Holding down the command or shift key adds rows to the selected.
    ...
    instance to SingleSelect constructor:
      ...
      new App.views.widgets.tilegrid.SingleSelect(@tilegrid)


    Events:
      # fired when a selected model is clicked and constructor option selectOneOnSelected==false
      @collection.trigger 'selectedClicked' modelId, evt


  ###
  class x.SingleSelect
    ...
    constructor: (@tilegrid, options={}) ->
      ...
      unless @collection.isSelectable
        throw "Dev: error: click and drag selection handlers for Tilegrid require a collection with the " +
          "SelectableCollection mixin."
    ...
    _getTileFromEvent:(evt) =>
      ...
      if $tile.length <= 0
        ...
        App.warn "dev: error: _getIndexFromEvent unable to find .tile for #{$target.selector}"
        return null
    ...
    # TODO: DRY this up with jquery helpers - slightly different.  If you implement it this way there,
    # scrolling up to vendors in zuStudio breaks
